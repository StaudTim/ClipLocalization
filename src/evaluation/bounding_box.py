...
class BoundingBox:
        ...
        if (type_coordinates == CoordinatesType.RELATIVE):
            ...
            if self._format == BBFormat.XYWH:
                (self._x, self._y, self._w,
                 self._h) = self._convert_to_absolute_values(img_size, coordinates)
                self._x2 = self._w
                self._y2 = self._h
    ...
    def get_relative_bounding_box(self, img_size=None):
        ...
        if img_size is not None:
            return self._convert_to_relative_values((img_size[0], img_size[1]),
                                                    (self._x, self._x2, self._y, self._y2))
        else:
            return self._convert_to_relative_values((self._width_img, self._height_img),
                                                    (self._x, self._x2, self._y, self._y2))
    ...
    @staticmethod
    def _convert_to_absolute_values(size, box):
        w_box = size[0] * box[2]
        h_box = size[1] * box[3]
    ...
    @staticmethod
    def _convert_to_relative_values(size, box):
        dw = 1. / (size[0])
        dh = 1. / (size[1])
        cx = (box[1] + box[0]) / 2.0
        cy = (box[3] + box[2]) / 2.0
