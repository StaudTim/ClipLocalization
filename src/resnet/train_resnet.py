...
def get_width_height(path, image_name):
...
def create_model(num_classes):
    """
    Creates a Resnet model with resnet18 as backbone.
    :param num_classes:
    :return: Returns the created model
    """
    backbone = torchvision.models.resnet18(pretrained=True)
    backbone.out_channels = 512  # Update the output channels for the backbone
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(image_files)):
        ...
        height, width = get_width_height(image_files[0], path_data)
        ...
        for epoch in range(epochs):
            metric_logger, batch_loss_list, batch_loss_cls_list, batch_loss_box_reg_list, batch_loss_objectness_list, batch_loss_rpn_list = \
                train_one_epoch(model, optimizer, train_loader, device, epoch, train_loss_hist, print_freq=100)

            loss_cls = torch.stack(batch_loss_cls_list).mean().item()
            loss_box_reg = torch.stack(batch_loss_box_reg_list).mean().item()
            loss_objectness = torch.stack(batch_loss_objectness_list).mean().item()
            loss_rpn = torch.stack(batch_loss_rpn_list).mean().item()

            loss_cls_list.append(loss_cls)
            loss_box_reg_list.append(loss_box_reg)
            loss_objectness_list.append(loss_objectness)
            loss_rpn_list.append(loss_rpn)

            log_in_csv(os.path.join(path_models, 'train.csv'), epoch, metric_logger, train_loss_hist, loss_cls, loss_box_reg, loss_objectness, loss_rpn)

            if epoch % 5 == 0:
                torch.save(model.state_dict(), os.path.join(path_models, model_name + '_' + str(epoch) + '.pth'))

            coco_evaluator, stats = evaluate(model, valid_loader, device)
            val_map_05.append(stats[0]["bbox"]["AP50"])
            val_map.append(stats[0]["bbox"]["AP"])
            log_in_csv(os.path.join(path_models, 'val.csv'), epoch, None, None, loss_cls, loss_box_reg, loss_objectness, loss_rpn,
                        stats[0]["bbox"]["AP50"], stats[0]["bbox"]["AP"])

            save_best_model.check(model, stats[0]["bbox"]["AP50"], path_models, model_name)
